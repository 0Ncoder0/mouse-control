const socket = io(window.socketIp);
...
const addKeyboardListener = (socket) => {
  ...
  const keys = ["123456789", "qwertyuiop", "asdfghjkl", "zxcvbnm"].map((ele) =>
    ele.split("")
  );
  const specialKeys = [
  ...
  const createKeyDiv = (key, value) => {
    ...
    div.innerText = key;
    div.addEventListener("click", () => socket.emit("key-tap", value || key));
    return div;
  };
  keyboard.innerHTML = "";
  {
  ...
  {
    directionKeys.forEach((row) => {
      const rowDiv = createRowDiv();
      row.forEach((item) => {
        const keyDiv = createKeyDiv(item.key, item.value);
        keyDiv.style.fontSize = "12vw";
        rowDiv.appendChild(keyDiv);
      });
      keyboard.appendChild(rowDiv);
    });
